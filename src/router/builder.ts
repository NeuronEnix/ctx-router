...
import type { TCtxConsumerFn } from "./types";
...
export type TRouteBuilder<TUserCTx extends TDefaultCtx> = {
   ...
   */
  via(...fns: Array<TCtxConsumerFn<TUserCTx>>): TRouteBuilder<TUserCTx>;

  /**
   * Registers the terminal handler for this route. Ends the chain.
   */
  to(handler: TCtxConsumerFn<TUserCTx>): void;
};
  ...
  implements TRouteBuilder<TUserCtx>
{
  constructor(
    ...
    private readonly middleware: Array<TCtxConsumerFn<TUserCtx>>
  ) {}
  ...
  via(...fns: Array<TCtxConsumerFn<TUserCtx>>): RouteBuilder<TUserCtx> {
    for (const fn of fns) {
