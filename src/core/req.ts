export type CtxReq<Data = Record<string, unknown>> = {
   ...
   * params < query < body (body has highest priority on key collisions).
   * Spread order is `{ ...params, ...query, ...body }` — last wins.
   */
  data: Data;
  ...
  route: {
     ...
     * Routes registered without an op match any op (wildcard).
     * A request without an op only matches op-less routes.
     */
    op?: string;
